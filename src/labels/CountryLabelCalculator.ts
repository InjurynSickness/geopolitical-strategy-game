...
export class CountryLabelCalculator {
    ...
    public async calculateLabelsAsync(
        ...
        const countryProvinceMap = new Map<string, string[]>();
    ...
    private async calculateLabelPosition(provinceIds: string[]): Promise<{ x: number; y: number } | null> {
        ...
        let scalingFactor = 1;
        if (size < 100) scalingFactor = 10;
        else if (size < 250) scalingFactor = 20;
        else if (size < 500) scalingFactor = 30;
        else if (size < 1000) scalingFactor = 40;
        else scalingFactor = 50;
        
        const gridWidth = Math.ceil(width / scalingFactor);
        const gridHeight = Math.ceil(height / scalingFactor);
        const grid: boolean[][] = Array(gridWidth).fill(null).map(() => Array(gridHeight).fill(false));
    ...
    private findLargestInscribedRectangle(grid: boolean[][]): { x: number; y: number; width: number; height: number } {
    ...
    private largestRectangleInHistogram(heights: number[]): { x: number; y: number; width: number; height: number } {
        const stack: number[] = [];
        let maxArea = 0;
        let largestRect = { x: 0, y: 0, width: 0, height: 0 };
    ...
    public getLabelCache(): Map<string, { x: number; y: number }> {
        return this.countryLabelCache;
    }
}