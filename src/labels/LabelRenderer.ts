...
export class LabelRenderer {
    ...
    public drawLabels(
        ...
        labelCache: Map<string, { x: number; y: number }>,
        provinceOwnerMap: Map<string, string>,
        cameraZoom: number
    ): void {
        const countrySizes = new Map<string, number>();
        ...
        for (const [countryId, position] of labelCache.entries()) {
            ...
            if (cameraZoom < 0.5 && countrySize < 100) continue;
            if (cameraZoom < 1.0 && countrySize < 30) continue;
            if (cameraZoom < 2.0 && countrySize < 10) continue;
            
            const sizeRatio = (countrySize - minSize) / (maxSize - minSize);
            const baseFontSize = 16 + (sizeRatio * 32);
            
            let fontSize = baseFontSize;
            if (cameraZoom < 0.5) fontSize *= 2.0;
            else if (cameraZoom < 1.0) fontSize *= 1.5;
            else if (cameraZoom > 3.0) fontSize *= 0.8;
